...
商品编辑页顶部提供“AI 一键改写”：选择字段、生成英文建议、对照、逐字段或全部采用、通过底部保存按钮保存。生成及预览均不写商品数据库。复用现有 `OPENAI_API_KEY` 和 `OPENAI_API_ENDPOINT`；`OPENAI_COPY_MODEL` 默认 `gpt-5.6-luna`，已在当前网关的模型列表和实际 Responses 请求中确认可用，也可配置为服务支持 Structured Outputs 的文本模型。

范围：名称、摘要、卖点、备用描述、详情区块标题/正文、SEO 标题/描述。网址、SKU、参数、类别、商业字段、图片不发送给模型作为可修改字段。发送模型的数据只有当前文案、SKU、分类、概览和规格；不包含价格成本、凭据或客户数据。使用 Responses API，`store: false`。
