...
- 生成文案省略单位缺失或依据不足的事实，不得把资料核查问题写成卖点、FAQ 或详情。中文核查事项只进入后台 warnings。校验拦截常见的待核实措辞及 HTML 转义字符；这不是完整的语义核实，仍须人工审核。已有此类内容须勾选对应字段使用“一键改写”清理，追加优化不会删除原文。
- 优化模式每次最多新增 5 条不重复卖点，总量受 100 条存储上限约束；已有 5 条不会阻止新增。没有新增时返回明确提示，原卖点保持不变。资料缺失问题应放在核查提示中，不作为面向客户的详情内容。
