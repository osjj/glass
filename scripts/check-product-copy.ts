...
async function main() {
  ...
  try {
    if (mode === "setup") {
      ...
      const result = await generateProductCopy({ copy: copySnapshot(product), fields: [...COPY_FIELDS], facts: { sku: product.sku ?? "", category: "Glass Bottles", overview: product.overviewFields.map(({ label, value }) => ({ label, value })), specifications: product.specifications.map(({ label, value }) => ({ label, value })) } });
