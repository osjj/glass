...
import { CustomerCaseStudy } from "@/components/site/customer-case-study";
...
export function CaseStudyArticle({ study, preview = false }: { study: CaseStudy; preview?: boolean }) {
  const baseUrl = getSiteUrl();
  const url = `${baseUrl}/case-studies/${study.slug}`;
  const studyLabel = study.kind === "customer-project" ? "Customer case study" : "Glarivo design study";
  ...
  const schema = [
    {
      ...
      dateModified: study.updatedAt, articleSection: studyLabel,
  ...
  return (
    <article className={theme.page}>
      {!preview && <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema).replace(/</g, "\\u003c") }} />}
      <ReadingHeader title={study.title} eyebrow={studyLabel} description={study.excerpt}
        image={study.kind === "design-study" ? "/images/home/editorial/hotel-glassware-case.webp" : study.coverImage} imageAlt={study.kind === "design-study" ? "Illustrative hospitality glassware setting for a design concept" : study.coverImageAlt ?? ""}
      ...
      {study.kind === "customer-project" ? <CustomerCaseStudy study={study} /> : <>
