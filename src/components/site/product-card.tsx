  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-[var(--line)] bg-white transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(15,42,89,0.12)]">
      <Link href={`/products/${product.slug}`} className="relative block aspect-[4/3] overflow-hidden bg-[var(--navy)]" aria-label={`View ${product.name}`}>
        <Image src={product.primaryImage} alt={product.primaryImageAlt} fill unoptimized loading={index === 0 ? "eager" : "lazy"} sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" className="object-cover transition duration-500 group-hover:scale-[1.03]" />
      </Link>
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mt-5 flex items-end justify-between gap-4 text-sm">
          <div>
            ...
            <strong className="mt-1 block text-lg text-[var(--navy)]">{product.price !== null && product.price > 0 ? `${product.currency} ${product.price.toFixed(2)}` : "Request pricing"}</strong>
          </div>
          <span className="text-right text-xs font-semibold text-[var(--ink-muted)]">{product.moq === null ? "MOQ on request" : `MOQ ${product.moq}${product.unit ? ` ${product.unit}` : ""}`}</span>
        </div>
