...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (image.size > MAX_IMAGE_BYTES) {
      return errorResponse("The source image must be 12 MB or smaller.", 413);
    }
    ...
    const uploaded = await uploadImageToR2({
      ...
      key: `case-studies/${folder}/${Date.now()}-${randomUUID()}.webp`,
    });

    return Response.json({
      image: {
        url: uploaded.url,
        alt: requestedAlt || fallbackAlt || "Case study image",
      },
    });
  } catch (error) {
    console.error("Case study image upload failed", error instanceof Error ? error.name : "UnknownError");
