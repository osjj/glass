import { CatalogImportSelectors } from "@/components/admin/catalog-import-selectors";
...
function importErrorMessage(error?: string, mappedCategory?: string) {
  if (error === "invalid-input") return "Select a maintained source category and an active Glarivo target category.";
  if (error === "invalid-source") return "That source category is missing, disabled, or no longer importable. Check the source-category library.";
  if (error === "inactive-category") return "The selected Glarivo category is missing or inactive.";
  if (error === "mapping-conflict") return `This source is already mapped to ${mappedCategory || "another category"}. Open category maintenance if the mapping needs to change.`;
  if (error === "preview-failed") return "The source could not be scanned. Check the category URL and network, then try again.";
...
export default async function ImportsPage({ searchParams }: ImportsPageProps) {
  ...
  const provider = query.provider === "SUNWIN" ? "SUNWIN" : "GARBO";
  const providerLabel = provider === "SUNWIN" ? "Sunwin" : "Garbo";
  const mode = query.mode === "publish" ? "publish" : query.mode === "draft" ? "draft" : provider === "SUNWIN" ? "draft" : "publish";
  const sourceFilter = /^(?:\/[a-z0-9]+(?:-[a-z0-9]+)*\/|\/product_category\/(?:9|10|11)\.html)$/.test(query.sourcePath || "")
    ? query.sourcePath
    : undefined;
  const [{ categoryOptions, sourceCategoryOptions, mappings }, { candidates }] = await Promise.all([
    getGarboImportSetup(provider),
    getImportCandidates(undefined, sourceFilter, 200, provider),
  ]);
  ...
  const publishedCount = candidates.filter((candidate) => candidate.product?.status === "PUBLISHED").length;
  const draftCount = candidates.filter((candidate) => candidate.product?.status === "DRAFT").length;
  const waitingCount = candidates.filter((candidate) => !candidate.product).length;
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <AdminHeader
        ...
        description="Choose a source, match its category to Glarivo, then scan and import as drafts or publish."
      />

      <nav className="mt-6 flex gap-3" aria-label="Import source">
        {(["GARBO", "SUNWIN"] as const).map((item) => <Link key={item} href={`/admin/imports?provider=${item}`} aria-current={provider === item ? "page" : undefined}
          className={provider === item ? "button-primary" : "button-secondary"}>{item === "SUNWIN" ? "Sunwin · Cosmetic Glass" : "Garbo · Glassware"}</Link>)}
      </nav>
      ...
      <section className="mt-7 overflow-hidden rounded-3xl border border-[#d7dcd8] bg-white">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-[var(--ink-muted)]">
                {providerLabel} and Glarivo categories remain independent. Previewing only scans the selected source and updates this source category&apos;s page/product counts; it does not create products.
              </p>
              {provider === "GARBO" ? <Link href="/admin/imports/garbo-categories" className="mt-2 inline-flex text-xs font-black text-[var(--accent-dark)] hover:underline">Maintain Garbo source categories</Link> : <Link href="/admin/categories" className="mt-2 inline-flex text-xs font-black text-[var(--accent-dark)] hover:underline">Manage cosmetic glass categories</Link>}
            </div>
          </div>
        </div>
        ...
        {categoryOptions.length && sourceCategoryOptions.length ? (
          <form action={previewGarboCategoryImport} className="grid gap-5 p-6 lg:grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)_180px_auto] lg:items-end lg:p-8">
            <CatalogImportSelectors key={`${provider}-${defaultSourceCategoryId}-${defaultCategoryId}`} sources={sourceCategoryOptions} targets={categoryOptions} defaultSource={defaultSourceCategoryId} defaultTarget={defaultCategoryId} provider={providerLabel} />
            ...
            <label className="text-sm font-black">Import mode
              <select className={inputClass} name="mode" defaultValue={mode}><option value="draft">Save as drafts</option><option value="publish">Import &amp; publish</option></select>
            </label>
            <p className="text-xs text-[var(--ink-muted)] lg:col-span-full">Scan covers all pages in this category. The batch limits the later product import / refresh. Existing published products keep their status in draft mode.</p>
          ...
          <div className="p-6 text-sm leading-6 text-[var(--ink-muted)] lg:p-8">
            Create an active target category and set up this source-category library before importing products. <Link href="/admin/imports/garbo-categories" className="font-black text-[var(--accent-dark)] hover:underline">Open Garbo categories</Link>
          </div>
        )}
      </section>
          ...
          <div className="grid gap-6 p-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center lg:p-8">
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-[var(--ink-muted)]">
                Found <strong className="text-[var(--ink)]">{query.products || "0"} products</strong> across {query.pages || "0"} category pages. Importing will save this mapping, copy authorized text and images, upload WebP media to R2, and {mode === "draft" ? "save new products as drafts" : "publish new products"} with request-quote pricing.
              </p>
            ...
            <form action={syncAndPublishGarboCategory}>
              ...
              <ImportSubmitButton mode={mode === "draft" ? "draft" : "import"} />
            </form>
          </div>
        </section>
      ) : null}
      ...
      <section className="mt-6 overflow-hidden rounded-3xl border border-[#d7dcd8] bg-white">
        <div className="flex flex-col gap-4 border-b border-[#e4e7e3] px-6 py-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-[var(--ink-muted)]">Open a linked product to edit it, or review the extracted source fields.</p>
          </div>
              ...
              <tbody className="divide-y divide-[#e4e7e3]">
                {candidates.map((candidate) => (
                  <tr key={candidate.id} className="text-sm align-top">
                    ...
                    <td className="px-5 py-4 text-xs font-bold text-[var(--ink-muted)]">{candidate.provider} · {candidate.sourceCategoryPath}</td>
                    <td className="px-5 py-4 text-xs leading-5 text-[var(--ink-muted)]">
                      ...
                      <Link href={`/admin/imports/${candidate.id}`} className="mt-2 block underline">Review source fields</Link>
                      <span className="block">{candidate.product?.status || candidate.status}</span>
                    </td>
