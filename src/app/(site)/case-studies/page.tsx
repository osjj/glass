...
export const metadata: Metadata = {
  ...
  description: "Explore glassware industry case reviews, with source attribution and practical notes on selection, customization and packaging.",
  alternates: { canonical: "/case-studies" },
  openGraph: {
    title: "Glassware Case Studies | Glarivo",
    description: "Glassware industry case reviews and practical buying lessons.",
    url: "/case-studies",
    type: "website",
  },
};
    ...
    <>
      <section className="border-b border-[var(--line)] bg-[var(--surface)] py-14 sm:py-20">
        <div className="site-container">
          ...
          <h1 className="mt-5 max-w-4xl text-balance text-4xl font-bold leading-[1.05] tracking-[-0.045em] text-[var(--navy)] sm:text-6xl">Glassware decisions,<br />in context.</h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--ink-muted)]">Explore industry examples through the decisions behind the product: selection, decoration and packaging.</p>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-[var(--ink-muted)]">Our first study reviews a publicly reported Garbo case. It is an independent editorial review, not a Glarivo customer project.</p>
        </div>
      </section>
      <section className="site-container py-12 sm:py-20" aria-label="Published case studies">
        {caseStudies.map((study) => (
          <article key={study.slug} className="grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
            <CaseStudyArtwork />
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.13em] text-[var(--blue)]">Industry case review · Garbo source</p>
              ...
              <Link href={`/case-studies/${study.slug}`} className="button-primary mt-8">Read the case review<ArrowRight size={18} weight="bold" aria-hidden="true" /></Link>
            </div>
          </article>
        ))}
      </section>
